  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 className="text-white text-sm sm:text-lg font-medium truncate">
          {video.name.replace(/\.[^/.]+$/, '')}
        </h2>
      </div>
      ...
      {/* Bottom controls */}
      <div
        className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent px-3 sm:px-6 pb-4 sm:pb-6 pt-10 sm:pt-16 transition-opacity duration-500 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={e => e.stopPropagation()}
      >
        {/* Progress bar */}
        <div
          ref={progressRef}
          className="relative w-full h-1.5 sm:h-1 bg-white/20 rounded-full cursor-pointer group mb-3 sm:mb-4 hover:h-2 transition-all"
          onClick={seek}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-4">
            {onPrev && <button onClick={onPrev} className="text-white/70 hover:text-white transition-colors hidden sm:block"><SkipBack className="h-5 w-5" /></button>}
            <button onClick={() => skip(-10)} className="text-white/70 hover:text-white transition-colors">
              <SkipBack className="h-4 w-4 sm:h-5 sm:w-5" />
            </button>
            <button onClick={togglePlay} className="text-white hover:text-white/80 transition-colors">
              {playing ? <Pause className="h-6 w-6 sm:h-7 sm:w-7" /> : <Play className="h-6 w-6 sm:h-7 sm:w-7 fill-current" />}
            </button>
            <button onClick={() => skip(10)} className="text-white/70 hover:text-white transition-colors">
              <SkipForward className="h-4 w-4 sm:h-5 sm:w-5" />
            </button>
            {onNext && <button onClick={onNext} className="text-white/70 hover:text-white transition-colors hidden sm:block"><SkipForward className="h-5 w-5" /></button>}
            ...
            <span className="text-white/70 text-[10px] sm:text-xs ml-1 sm:ml-2">
                ...
                <div className="absolute bottom-full right-0 mb-2 bg-[#1a1a1a] rounded py-1 shadow-xl">
                  {speeds.map(s => (
                    <button key={s} onClick={() => { setSpeed(s); setShowSpeed(false); }}
                      className={`block w-full px-4 py-1.5 text-xs text-left hover:bg-white/10 transition-colors ${s === speed ? 'text-white' : 'text-white/60'}`}>{s}x</button>
                  ))}
                </div>
              )}
            </div>
